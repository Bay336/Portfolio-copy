...
//  Чтобы показать (открыть) меню   
   if (navOpen) {
       navOpen.addEventListener('click', () => {
        navMenu.classList.add('show-menu');
    })
   };
//  Чтобы скрыть (закрыть) меню
    if (navClose) {
        navClose.addEventListener('click', () => {
         navMenu.classList.remove('show-menu');
        })
    };
//  Чтобы при клики на любую иконку меню скрывалось 
const navLink = document.querySelectorAll('.nav-link');
  ...
  function scrollHeader() {
    ...
    if (this.scrollY >= 80) nav.classList.add('scroll-header')   //при скроле больше 80px будет применяться класс scroll-header(подсветка)..
     else nav.classList.remove ('scroll-header')                 //если больше 80px то подсветка удалится...
  }

  window.addEventListener('scroll', scrollHeader);

  //! Смена темы: темную  на светлую (смена иконок)
...
tabs.forEach(tab => {
  ...
  //! Portfolio swiper skript
   let swiperPortfolio = new Swiper('.portfolio-container', {
    grapCursor: true,        //можем переключать мышкой
    loop: true,                //бесконечный цикл

    navigation: {
        nextEl: '.swiper-button-next',   //нажатие правой кнопки
        prevEl: '.swiper-button-prev',   //нажатие левой кнопки
    },
    ...
    path.style.transition = path.styleWebkitTransition = "stroke-dshoffset 10ms linear";
